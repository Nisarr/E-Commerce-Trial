...
import { TrendingUp, Trash2, Plus, Loader2, Package, Search, Zap, X } from 'lucide-react';
import { useSearchParams } from 'react-router-dom';
import { getProducts, updateProduct, syncTags } from '../../services/api';
import type { Product } from '../../types';

export const BestSellingManager: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const [taggedRes, allRes] = await Promise.all([
        getProducts({ tag: 'best-selling', limit: 100 }),
  ...
  const handleToggle = async (product: Product) => {
    try {
      ...
      if (currentTags.includes('best-selling')) {
        newTags = currentTags.filter(t => t !== 'best-selling');
      } else {
        newTags = Array.from(new Set([...currentTags, 'best-selling']));
      }
  ...
  return (
    <div className="space-y-4 md:space-y-8 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        <div>
          <h2 className="text-xl md:text-3xl font-black text-primary tracking-tight">Best Selling</h2>
          <p className="text-[10px] md:text-sm text-muted font-medium mt-0.5">Featured in the "Best Sellers" section.</p>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
          <button 
            onClick={handleAutoSync}
            disabled={syncing}
            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 md:px-6 py-2.5 md:py-3 bg-indigo-500 text-white rounded-xl md:rounded-2xl font-black uppercase tracking-widest text-[10px] md:text-xs transition-all shadow-lg shadow-indigo-200 hover:bg-indigo-600 disabled:opacity-50"
          >
        ...
        <div className="bg-white rounded-[2rem] p-8 shadow-xl shadow-primary/5 border border-orange-100 animate-in slide-in-from-top-4 duration-500">
          <div className="flex items-center gap-4 mb-6">
            <div className="relative flex-grow">
              ...
              <input 
                type="text"
                placeholder="Search products to force into Best Selling..."
                value={searchTerm}
                onChange={(e) => {
              ...
              return (
                <div 
                  ...
                  className={`p-4 rounded-2xl border-2 transition-all flex items-center gap-4 ${
                    isTagged ? 'border-indigo-500 bg-indigo-50/30' : 'border-gray-100 hover:border-indigo-200'
                  }`}
                >
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <div className="text-[10px] font-black text-muted uppercase tracking-widest">Sold: {product.soldCount}</div>
                  </div>
                  <button 
                    onClick={() => handleToggle(product)}
                    className={`p-2 rounded-xl transition-all ${
                      isTagged ? 'bg-indigo-500 text-white' : 'bg-gray-100 text-muted hover:bg-indigo-100 hover:text-indigo-600'
                    }`}
                  >
      ...
      <div className="bg-white md:rounded-[2.5rem] md:shadow-xl shadow-primary/5 md:border border-gray-100 overflow-hidden">
        ...
        <table className="w-full text-left hidden md:table">
          <thead className="bg-gray-50/50 border-b border-gray-100">
            <tr>
              <th className="px-8 py-5 font-black text-primary text-[10px] uppercase tracking-[0.2em]">Active Best Sellers</th>
              <th className="px-8 py-5 font-black text-primary text-[10px] uppercase tracking-[0.2em]">Sales Count</th>
                ...
                <tr key={product.id} className="hover:bg-gray-50/30 transition-colors group">
                    ...
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="px-2 py-0.5 bg-indigo-100 text-indigo-600 rounded text-[9px] font-black uppercase tracking-tighter">BEST SELLER</span>
                  ...
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      <span className="text-base font-black text-primary">{product.soldCount}</span>
                      <span className="text-[10px] text-muted font-bold uppercase tracking-widest">units sold</span>
                    </div>
                  </td>
                  <td className="px-8 py-5">
              ...
              <tr>
                <td colSpan={3} className="px-8 py-20 text-center">
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <div>
                      <p className="font-black text-primary uppercase tracking-widest text-sm">No best sellers tagged</p>
        ...
        <div className="md:hidden divide-y divide-gray-100">
              ...
              <div key={product.id} className="p-4 space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-grow min-w-0">
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-indigo-600">{product.soldCount} Units Sold</span>
          ...
          {items.length === 0 && (
            <div className="p-12 text-center text-gray-400">
              ...
              <p className="text-xs font-black uppercase tracking-widest">No best sellers</p>
            </div>
          )}
        </div>
      </div>
      
      <div className="bg-indigo-50/50 rounded-2xl md:rounded-3xl p-4 md:p-6 border-2 border-dashed border-indigo-200 flex items-center gap-4 md:gap-6">
        ...
        <div>
          ...
          <p className="text-[10px] md:text-xs text-muted font-medium mt-0.5 md:mt-1">
            "Auto Sync" picks the <span className="font-bold text-indigo-600">8 products with the highest sales</span>. 
            You can manually "Force" any product to be a Best Seller.
          </p>
        </div>
      </div>
    </div>
