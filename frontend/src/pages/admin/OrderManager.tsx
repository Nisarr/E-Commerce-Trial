    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {Object.entries(statusCounts).map(([status, count]) => (
          ...
          >
            {status}
            <span className={`px-1.5 py-0.5 rounded-md text-[8px] ${statusFilter === status ? 'bg-white/20' : 'bg-gray-100'}`}>
              {count}
            </span>
          </button>
        ))}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-md flex items-end md:items-center justify-center p-0 md:p-4">
          <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] w-full max-w-6xl h-[85vh] md:h-auto md:max-h-[95vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom md:zoom-in-95 duration-300 mb-18 md:mb-0">
            {/* Header */}
            <div className="px-5 md:px-8 py-5 md:py-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gray-50/50 gap-4 flex-shrink-0">
              <div className="flex items-center gap-3 md:gap-4 w-full sm:w-auto">
                ...
                <div className="min-w-0">
                  <h2 className="text-lg md:text-2xl font-black font-garamond text-primary flex items-center gap-2 truncate">
                    ...
                    >
                      {copiedId === 'inv' ? <Check size={14} className="text-green-600" /> : <Copy size={12} className="text-gray-400" />}
                    </button>
                  </h2>
                ...
                <button onClick={() => setSelectedOrder(null)} className="sm:hidden ml-auto p-2 hover:bg-gray-200 rounded-full text-gray-400">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex items-center gap-2 md:gap-3 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-hide">
                ...
                <button 
                  ...
                  className={`px-3 md:px-5 py-2 md:py-2.5 rounded-xl font-bold transition-all flex items-center gap-1.5 md:gap-2 text-[10px] md:text-sm shadow-sm whitespace-nowrap flex-shrink-0 ${
                    isEditing ? 'bg-green-500 text-white hover:bg-green-600 shadow-lg shadow-green-500/20' : 'bg-white border border-gray-200 text-primary hover:bg-gray-50'
                  }`}
                >
                ...
                <button 
                  ...
                  className="px-3 md:px-5 py-2 md:py-2.5 bg-white border border-gray-200 text-primary rounded-xl font-bold hover:bg-gray-50 transition-all flex items-center gap-1.5 md:gap-2 text-[10px] md:text-sm shadow-sm whitespace-nowrap flex-shrink-0"
                >
                  <ExternalLink className="w-3.5 h-3.5 md:w-4 md:h-4" /> Invoice
                </button>
                <button onClick={() => setSelectedOrder(null)} className="hidden sm:flex p-2 md:p-2.5 hover:bg-white rounded-full transition-colors text-gray-400 border border-transparent hover:border-gray-100 shadow-sm">
                  <X size={24} />
                </button>
              </div>
            </div>
            ...
            <div className="p-5 md:p-8 overflow-y-auto flex-grow bg-white custom-scrollbar pb-8 md:pb-8">
              {loadingDetails ? (
                ...
                <div className="lg:col-span-7 space-y-8">
                  ...
                  {/* Progress Stepper */}
                  <div className="bg-gray-50/50 rounded-2xl md:rounded-[2rem] p-4 md:p-6 border border-gray-100 overflow-x-auto scrollbar-hide">
                        ...
                        return (
                          <div key={step} className="flex flex-col items-center gap-2 flex-1 relative">
                            ...
                            <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center z-10 transition-all duration-500 ${
                              isCompleted ? 'bg-primary text-white' : isActive ? 'bg-accent text-white scale-110 shadow-lg' : 'bg-gray-200 text-gray-500'
                            }`}>
                            ...
                            <span className={`text-[8px] md:text-[10px] font-black uppercase tracking-tighter ${isActive ? 'text-primary' : 'text-gray-400'}`}>{step}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Customer Info */}
                    <div className="bg-white rounded-2xl md:rounded-[2rem] p-5 md:p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                      <h3 className="text-[9px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                        <Phone className="text-primary w-3 h-3 md:w-3.5 md:h-3.5" /> Contact
                      </h3>
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 md:w-10 md:h-10 bg-gray-50 rounded-lg md:rounded-xl flex items-center justify-center text-gray-500 flex-shrink-0">
                            ...
                            <div className="min-w-0">
                              <button 
                                ...
                                className="font-bold text-primary hover:text-accent transition-colors text-left text-sm md:text-base truncate block w-full"
                              >
                                {selectedOrder.customerName}
                              </button>
                              <div className="flex items-center gap-1.5 truncate">
                                <Mail size={10} className="text-gray-400 flex-shrink-0" />
                                <span className="text-[10px] md:text-xs text-gray-500 truncate">{selectedOrder.customerEmail || 'No email'}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0">
                            <Phone className="text-gray-400 flex-shrink-0 w-3 h-3 md:w-3.5 md:h-3.5" />
                            <input 
                              type="text"
                              value={isEditing ? editForm.customerPhone : selectedOrder.customerPhone}
                              onChange={(e) => setEditForm({...editForm, customerPhone: e.target.value})}
                              disabled={!isEditing}
                              className={`text-xs md:text-sm font-bold text-primary bg-transparent outline-none truncate w-full ${isEditing ? 'border-b border-primary' : ''}`}
                            />
                          </div>
                          <button 
                          ...
                          >
                            {copiedId === 'phone' ? <Check size={14} className="text-green-600" /> : <Copy className="text-gray-400 w-3 h-3 md:w-3.5 md:h-3.5" />}
                          </button>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl md:rounded-[2rem] p-5 md:p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="text-[9px] md:text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                          <MapPin className="text-primary w-3 h-3 md:w-3.5 md:h-3.5" /> Shipping Address
                        </h3>
                        <button 
                          onClick={() => copyToClipboard(isEditing ? editForm.shippingAddress : selectedOrder.shippingAddress, 'addr')}
                          className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors"
                        >
                          {copiedId === 'addr' ? <Check size={14} className="text-green-600" /> : <Copy className="text-gray-400 w-3 h-3 md:w-3.5 md:h-3.5" />}
                        </button>
                      </div>
                      <textarea 
                        ...
                        disabled={!isEditing}
                        className={`w-full text-xs md:text-sm text-gray-600 bg-transparent outline-none resize-none min-h-[60px] md:min-h-[80px] leading-relaxed ${isEditing ? 'border-2 border-primary/20 rounded-xl p-3 bg-primary/5' : ''}`}
                      />
                    </div>
                  </div>

                  {/* Payment Details */}
                  <div className="bg-white rounded-2xl md:rounded-[2rem] p-5 md:p-6 border border-gray-100 shadow-sm overflow-x-auto">
                    <h3 className="text-[9px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <CreditCard className="text-primary w-3 h-3 md:w-3.5 md:h-3.5" /> Payment
                    </h3>
                    <div className="flex flex-wrap items-center gap-3 md:gap-4 min-w-max">
                      ...
                      {selectedOrder.paymentPhone && (
                        <div className="px-3 md:px-4 py-2 md:py-3 bg-gray-50 rounded-xl md:rounded-2xl border border-gray-100 flex items-center gap-3">
                          <div className="text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">Sender</div>
                          <span className="font-mono font-bold text-xs md:text-sm text-primary">{selectedOrder.paymentPhone}</span>
                          <button onClick={() => copyToClipboard(selectedOrder.paymentPhone!, 'payp')} className="text-gray-400 hover:text-primary">
                      ...
                      {selectedOrder.paymentTrxId && (
                        <div className="px-3 md:px-4 py-2 md:py-3 bg-gray-50 rounded-xl md:rounded-2xl border border-gray-100 flex items-center gap-3">
                          <div className="text-[8px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest">TRXID</div>
                          <span className="font-mono font-bold text-xs md:text-sm text-primary">{selectedOrder.paymentTrxId}</span>
                          <button onClick={() => copyToClipboard(selectedOrder.paymentTrxId!, 'payt')} className="text-gray-400 hover:text-primary">
                  ...
                  {/* Items List */}
                  <div className="bg-white rounded-2xl md:rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-sm">
                    <div className="px-5 md:px-6 py-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                      <h3 className="text-[9px] md:text-xs font-black text-gray-500 uppercase tracking-widest">Order Items</h3>
                      <span className="text-[10px] md:text-xs font-bold text-primary">{selectedOrder.items?.length} Items</span>
                    </div>
                    <div className="divide-y divide-gray-50 max-h-[300px] overflow-y-auto custom-scrollbar">
                      {selectedOrder.items?.map((item, idx) => (
                        <div key={idx} className="p-3 md:p-4 flex items-center justify-between hover:bg-gray-50/50 transition-colors gap-3">
                          <div className="flex items-center gap-3 md:gap-4 flex-1 min-w-0">
                            <div className="w-10 h-10 md:w-12 md:h-12 bg-gray-100 rounded-lg md:rounded-xl flex items-center justify-center text-gray-400 flex-shrink-0 p-0.5">
                            ...
                            <div className="min-w-0">
                              <p className="font-bold text-primary text-xs md:text-sm truncate">{item.productName || `ID: ${item.productId.slice(0,8)}`}</p>
                    ...
                    <div className="px-5 md:px-6 py-4 bg-gray-50/50 flex justify-between items-center border-t border-gray-100">
                      <span className="font-bold text-gray-500 text-sm">Grand Total</span>
                      <span className="text-lg md:text-xl font-black text-primary">৳{selectedOrder.totalAmount.toLocaleString()}</span>
                    </div>
                  </div>
                ...
                <div className="lg:col-span-5 space-y-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="sm:col-span-2">
                          ...
                          <select 
                            ...
                            className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white outline-none focus:bg-white focus:text-primary transition-all text-xs md:text-sm font-bold appearance-none"
                          ...
                          >
                            ...
                            <option value="Processing" className="text-primary">Processing</option>
                            <option value="Shipped" className="text-primary">Shipped</option>
                            <option value="Out for Delivery" className="text-primary">Out for Delivery</option>
                            <option value="Delivered" className="text-primary">Delivered</option>
                          </select>
                        </div>
                        <div className="sm:col-span-2">
                          ...
                          <input 
                            ...
                            className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder:text-white/40 outline-none focus:bg-white focus:text-primary transition-all text-xs md:text-sm font-medium"
                          />
                        </div>
                      </div>
                      <button 
                        type="submit"
                        className="w-full py-3 md:py-3.5 bg-accent text-white rounded-xl font-black uppercase tracking-widest hover:bg-white hover:text-accent transition-all shadow-lg text-[10px] md:text-xs"
                      >
                        Update & Notify
                      </button>
                    </form>
                  ...
                  <div className="bg-white rounded-2xl md:rounded-[2rem] p-5 md:p-6 border border-gray-100 shadow-sm space-y-4">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-[9px] md:text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                        <Lock className={`w-3 h-3 md:w-3.5 md:h-3.5 ${isAdminEditing ? "text-accent" : "text-primary"}`} /> Admin Controls
                      </h3>
                      {!isAdminEditing ? (
                        <button 
                          onClick={() => setIsAdminEditing(true)}
                          className="px-2 md:px-3 py-1 md:py-1.5 bg-gray-50 text-gray-600 rounded-lg text-[8px] md:text-[9px] font-black uppercase tracking-widest hover:bg-gray-100 transition-all border border-gray-100 flex items-center gap-1 md:gap-1.5"
                        >
                        ...
                        <div className="flex items-center gap-2">
                          <button 
                            ...
                            className="px-2 py-1 text-gray-400 hover:text-gray-600 text-[8px] md:text-[9px] font-black uppercase tracking-widest transition-all"
                          >
                            Cancel
                          </button>
                          <button 
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
                        <div className="flex-1">
                          <label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-2">
                            <Hash className="text-primary w-2.5 h-2.5 md:w-3 md:h-3" /> Courier Tracking ID
                          </label>
                          <input 
                            type="text"
                            ...
                            className={`w-full px-3 md:px-4 py-2 md:py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-xs md:text-sm font-bold outline-none focus:border-primary transition-all ${isAdminEditing ? 'bg-white shadow-sm ring-2 ring-primary/5' : ''}`}
                          />
                        </div>
                        <div className="flex-1">
                          <label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-2">
                            <ExternalLink className="text-primary w-2.5 h-2.5 md:w-3 md:h-3" /> Tracking Link (URL)
                          </label>
                          <input 
                            type="text"
                      ...
                      <div className="pt-4 border-t border-gray-50">
                        <label className="block text-[9px] md:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-2">
                          <MessageSquare className="text-primary w-2.5 h-2.5 md:w-3 md:h-3" /> Internal Note (Admin Only)
                        </label>
                        <textarea 
                          ...
                          disabled={!isAdminEditing}
                          className={`w-full px-3 md:px-4 py-2 md:py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-xs md:text-sm outline-none focus:border-primary resize-none h-20 md:h-24 transition-all ${isAdminEditing ? 'bg-white shadow-sm ring-2 ring-primary/5' : ''}`}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white rounded-2xl md:rounded-[2rem] p-5 md:p-6 border border-gray-100 shadow-sm flex flex-col h-[280px] md:h-[320px]">
                    <h3 className="text-[9px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-6 flex-shrink-0">Activity Log</h3>
                    <div className="overflow-y-auto pr-2 custom-scrollbar flex-grow">
                        ...
                        {selectedOrder.trackings?.map((track, idx) => (
                          <div key={idx} className="relative pl-8">
                            ...
                            <div className="flex justify-between items-start">
                              ...
                              <span className="text-[8px] md:text-[10px] font-bold text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full whitespace-nowrap">
                                {track.createdAt ? new Date(track.createdAt).toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', month: 'short', day: 'numeric' }) : 'N/A'}
                              </span>
                            </div>
                            {track.location && (
                              <p className="text-[9px] md:text-[10px] text-primary/60 mt-1 font-bold flex items-center gap-1 truncate">
                      ...
                      {(!selectedOrder.trackings || selectedOrder.trackings.length === 0) && (
                        <p className="text-xs text-gray-500 font-medium opacity-60">No activity logged yet.</p>
                      )}
                    </div>
                  </div>
                </div>
